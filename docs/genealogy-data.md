...
C20C added Michael's six supported aunts and uncles only through parent-child edges to the corresponding grandparents. C24 records Michael's later confirmation of all twelve grandparent-to-aunt/uncle parent roles. Those relationship claims are now verified and family-confirmed while retaining their existing documentary provenance; the documentary evidence for the Edmond and Allen Sr. roles remains indirect when considered alone. Aunt, uncle, and sibling labels are never stored as relationship edges.

C20D admitted nine identified first cousins through actual parent-child structure. `firstCousins(id)` derives each cousin only through subject → parent → shared grandparent → parent's sibling → cousin. C24 adds Michael's family confirmation to Cathy→Paige, Cathy→Sean, and all seven maternal cousin parent assignments, making those relationship roles verified while retaining the earlier documentary provenance and its indirect-evidence notes. Richard Russell McRae's memorial independently supports his two verified/documented parent edges to Paige and Sean. Sid Roger resolves to the already accepted Sidney Paul Roger sibling record. Lauren Dugas and Collin Adkisson have no canonical person records, relationships, or placement. Unresolved Ryan/Ryan Earl forms remain source-attributed name variants of the Rhyan record and are not treated as a proved identity merge.
